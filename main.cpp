...
bool extract_trajectory_from_video(
    ...
    cv::Mat& out_background_frame) //获取背景
{
    ...
    while (cap.read(frame)) {
        ...
        if (frame_count == 0) {
            out_background_frame = frame.clone();
        }
        ...
        for (const auto& contour : contours) {
            ...
            if (area > max_area && area > MIN_CONTOUR_AREA) {
                max_area = area;
                largest_contour_rect = cv::boundingRect(contour);
                found = true;
            }
        }
        ...
        if (found) {
            ...
            double current_x = largest_contour_rect.x + largest_contour_rect.width / 2.0;
            double current_y = frame.rows - (largest_contour_rect.y + largest_contour_rect.height / 2.0);

            t_data.push_back(current_t);
            x_data.push_back(current_x);
    ...
    if (t_data.size() < 10) {
        std::cerr << "Error: Found fewer than 10 trajectory points." << std::endl;
        return false;
    }
